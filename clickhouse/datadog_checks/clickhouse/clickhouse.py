...
class ClickhouseCheck(AgentCheck):
    ...
    def __init__(self, name, init_config, instances):
        ...
        self._collection_methods = (
            self.query_system_metrics,
            self.query_system_events,
            self.query_system_asynchronous_metrics,
            self.query_system_parts,
            self.query_system_dictionaries,
        )
    ...
    def query_system_metrics(self):
        ...
        result = self.execute_query_schema_search(queries.SystemMetrics)
    ...
    def query_system_events(self):
        ...
        self.execute_query_schema_search(queries.SystemEvents)

    def query_system_asynchronous_metrics(self):
        # https://clickhouse.yandex/docs/en/operations/system_tables/#system_tables-asynchronous_metrics
        self.execute_query_schema_search(queries.SystemAsynchronousMetrics)

    def query_system_parts(self):
        # https://clickhouse.yandex/docs/en/operations/system_tables/#system_tables-parts
        for part in self.execute_query_defined_schema(queries.SystemParts):
            tags = ['database:{}'.format(part['database']), 'table:{}'.format(part['table'])]
            tags.extend(self._tags)

            self.gauge('table.mergetree.size', part['bytes'], tags=tags)
            self.gauge('table.mergetree.part.current', part['parts'], tags=tags)
            self.gauge('table.mergetree.row.current', part['rows'], tags=tags)

    def query_system_dictionaries(self):
        # https://clickhouse.yandex/docs/en/operations/system_tables/#system-dictionaries
        for dictionary in self.execute_query_defined_schema(queries.SystemDictionaries):
            tags = ['dictionary:{}'.format(dictionary['name'])]
            tags.extend(self._tags)

            self.gauge('dictionary.memory.used', dictionary['bytes_allocated'], tags=tags)
            self.gauge('dictionary.item.current', dictionary['element_count'], tags=tags)
            self.gauge('dictionary.load', dictionary['load_factor'], tags=tags)
    ...
    def execute_query_schema_search(self, query):
    ...
    def execute_query_defined_schema(self, query):
        rows = self.execute_query_raw(query.query, lambda: ', '.join(sorted(query.views)))
        ...
        for row in rows:
            for column, value in zip(query.columns, row):
                result[column] = value
